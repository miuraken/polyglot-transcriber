...
def transcribe_audio(input_audio_path, output_text_path, primary_lang, secondary_lang, min_speech_duration_ms, min_silence_duration_ms, lang_prob_threshold, vad_aggressiveness):
    ...
    with open(output_text_path, "w", encoding="utf-8") as f:
        ...
        for i, (segment_start_time_abs, segment_end_time_abs, segment_bytes) in enumerate(vad_segment_generator((sample_rate, audio_data), vad_aggressiveness, min_speech_duration_ms, min_silence_duration_ms)):
            ...
            for segment_whisper in transcribe_result["segments"]:
                ...
                print(f"  {format_time(segment_start_time_abs)} - {format_time(segment_end_time_abs)}: {display_lang_str}{confidence_str} - {first_30_chars}")
